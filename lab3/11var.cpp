...
void prep(int str_num, int n, int m, int s, double *matrix, double *sviortka, double* results){
    ...
    // теперь есть матрица и матрица свёртки
    ...
    for (int i = 0; i < m - 2; i++){
        results[str_num * (m - s + 1) + i] = mresults[i];
    }
}
...
int main(){
    ...
    if(n < n_errosion || n < n_blinking || m < n_errosion || m < n_blinking){
        cout << "Error, incorrect matrix sizes entered";
        return 0;
    }
